...
class CGIRootFix(object):
    ...
    or CGI and you have problems with your backend root being set to the CGI
    script's path instead of the path users are going to visit.

    :param app: the WSGI application
    :param app_root: Defaulting to ``'/'``, you can set this to
        something else if your backend is mounted somewhere else.
...
class HeaderRewriterFix(object):
    ...
    not or to add `X-Powered-By` headers::
        ...
        backend = HeaderRewriterFix(backend, remove_headers=['Date'],
